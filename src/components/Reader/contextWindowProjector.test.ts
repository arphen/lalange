...
describe('ContextWindowProjector', () => {
    it('shifts the window by reusing existing word nodes', () => {
        ...
        const initial = projector.project(container, words, 0, 3, () => 'text-gray-400');
        const stableWord = container.querySelector('[data-index="1"]');

        const shifted = projector.project(container, words, 1, 4, () => 'text-sky-500');
    ...
    it('rebuilds after external DOM replacement and keeps text inert', () => {
        ...
        projector.project(container, words, 0, 1, () => 'text-gray-400');
        container.innerHTML = '<span>stale river</span>';

        const result = projector.project(container, words, 0, 1, () => 'text-gray-400');
