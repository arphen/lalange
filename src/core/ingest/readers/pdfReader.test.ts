...
describe('PdfIngestReader', () => {
    it('uses injected parsing and preserves meaningful pages in source order', async () => {
        ...
        expect(chapters[0].slices[1].html).toContain('data-pdf-page="3"');
    });
    ...
    it('rejects scanned or image-only PDFs instead of creating an empty book', async () => {
        ...
        await expect(reader.prepareInitial(new File(['%PDF-1.7'], 'scan.pdf')))
