...
describe('bounded repair protocol', () => {
    it('builds bounded context while retaining exact candidate offsets', async () => {
        ...
        expect(buildRepairPrompt(candidate, context)).toContain(`candidateId: ${candidate.id}`);
    });

    it('accepts only strict JSON proposal objects', () => {
        ...
        expect(() => parseRepairProposal('{"candidateId":"candidate-1","action":"replace","reasonCode":"uncertain","extra":true}')).toThrow(/unknown fields/i);
    });
